...
harmonics-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. harmonics-cli
...
#   - consumer-identifying prose: `guildmaster` → `harmonics-cli` (NOT
...
If a re-sync would lose a harmonics-cli adaptation, lift the change
...
#     → `which harmonics-cli vendors from guildmaster`
