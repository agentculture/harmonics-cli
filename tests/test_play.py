...
def test_play_device_flag_is_passed_to_backend(monkeypatch: pytest.MonkeyPatch) -> None:
    captured, fake_play = _make_capture()
    monkeypatch.setattr("harmonics.audio.play", fake_play)

    rc = main(["play", "--intent", "success", "--play", "--device", "pipewire"])

    assert rc == 0
    assert captured["device"] == "pipewire"


def test_play_device_env_var_used_when_no_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    captured, fake_play = _make_capture()
    monkeypatch.setattr("harmonics.audio.play", fake_play)
...
def test_play_device_flag_overrides_env_var(monkeypatch: pytest.MonkeyPatch) -> None:
    captured, fake_play = _make_capture()
    monkeypatch.setattr("harmonics.audio.play", fake_play)
    monkeypatch.setenv("HARMONICS_AUDIO_DEVICE", "pulse")

    rc = main(["play", "--intent", "success", "--play", "--device", "hw:1"])

    assert rc == 0
    assert captured["device"] == "hw:1"


def test_play_device_defaults_to_none(monkeypatch: pytest.MonkeyPatch) -> None:
    captured, fake_play = _make_capture()
    monkeypatch.setattr("harmonics.audio.play", fake_play)
