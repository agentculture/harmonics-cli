"""Smoke tests for the harmonics-cli CLI entry point and its verbs."""
...
from harmonics.explain import known_paths
...
def test_no_args_prints_help(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main([])
    assert rc == 0
    assert "usage: harmonics-cli" in capsys.readouterr().out
...
def test_whoami_text(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "nick: harmonics-cli" in out
...
def test_whoami_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert payload["nick"] == "harmonics-cli"
...
def test_learn_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert rc == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["tool"] == "harmonics-cli"
    assert payload["version"] == __version__
...
def test_explain_root(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main(["explain"])
    assert rc == 0
    assert "# harmonics-cli" in capsys.readouterr().out


def test_explain_self(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main(["explain", "harmonics-cli"])
    assert rc == 0
...
def test_explain_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "harmonics-cli whoami" in payload["markdown"]
