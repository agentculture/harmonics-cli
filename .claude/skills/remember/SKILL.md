...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope harmonics-cli
...
scope:** the default personal scope is **private** (`--scope harmonics-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `harmonics-cli`). Private records are served only to a recall in the same
